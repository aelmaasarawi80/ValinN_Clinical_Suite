...
export default function App() {
  ...
  const runLocalCalculation = () => {
    ...
    if (numGroups === 1) {
      ...
      report = `The sample size was calculated to evaluate the descriptive prevalence of ${primaryOutcome} in ${groupNames[0]}. To achieve a ±${d * 100:.0f}% margin of error with ${(confidenceLevel*100).toFixed(0)}% confidence, assuming expected prevalence of ${prevProp}%${doiCitation} (Cochran, 1977), the baseline calculation required ${n_per_group} total samples.`;
    } else {
      if (dataSource === "none") {
